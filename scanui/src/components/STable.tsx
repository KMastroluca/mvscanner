...
// STableAction imported but never used
import {
  STableProps,
  STableData,
  STableColumn,
...
// SortDataType imported but never used
import {
  SortType,
  StringSort,
  NumericalSort,
...
import { PopoverContentProps } from "../types/componentTypes";
...
export const STable: Component<STableProps> = (props: STableProps) => {
  ...
  const [priorityTableData, setPriorityTableData] = createSignal<
    STableRowItem[] | null
  >(null);

  const [tableColumns, setTableColumns] = createSignal<STableColumn[] | null>(
    null
  );
  const [tableAllSelect, setTableAllSelect] = createSignal({
    allSelected: false,
  });

  const [tableSelectedItems, setTableSelectedItems] = createSignal<
    STableRowItem[]
  >([]);
  ...
  const [previousSort, setPreviousSort] = createSignal<{
    type: SortType;
    columnIndex: number;
  } | null>(null);
  ...
  const handleSelectRow = (e: Event, item: STableRowItem | null) => {
    ...
    if (tableData() !== null) {
      ...
      if (item === null) {
        console.log(
          "Selected AllSelect Toggle, ",
          !tableAllSelect().allSelected
        );
        ...
        if (rowIndex !== -1) {
          tableCpy[rowIndex] = {
            ...tableCpy[rowIndex],
            selected: !tableCpy[rowIndex].selected,
          };
        }
      }
  ...
  const toggleCurrentSort = () => {
    if (currentSort() !== null) {
      if (
        tableColumns()![currentSort()!.columnIndex].sortType!.type === "String"
      ) {
        if (currentSort()!.type === StringSort.Alphabetical) {
          setCurrentSort({
            columnIndex: currentSort()!.columnIndex,
            type: StringSort.AlphaReverse,
          });
        } else if (currentSort()!.type === StringSort.AlphaReverse) {
          setCurrentSort({
            columnIndex: currentSort()!.columnIndex,
            type: StringSort.Alphabetical,
          });
        }
      } else if (
        tableColumns()![currentSort()!.columnIndex].sortType!.type ===
        "Numerical"
      ) {
        if (currentSort()!.type === NumericalSort.Ascending) {
          setCurrentSort({
            columnIndex: currentSort()!.columnIndex,
            type: NumericalSort.Descending,
          });
        } else if (currentSort()!.type === NumericalSort.Descending) {
          setCurrentSort({
            columnIndex: currentSort()!.columnIndex,
            type: NumericalSort.Ascending,
          });
        }
      } else if (
        tableColumns()![currentSort()!.columnIndex].sortType!.type === "Date"
      ) {
        if (currentSort()!.type == DateSort.LatestFirst) {
          setCurrentSort({
            columnIndex: currentSort()!.columnIndex,
            type: DateSort.OldestFirst,
          });
        } else if (currentSort()!.type == DateSort.OldestFirst) {
          setCurrentSort({
            columnIndex: currentSort()!.columnIndex,
            type: DateSort.LatestFirst,
          });
        }
      }
    } else {
      console.error(
        "[-] This should never happen, we tried to toggle sort, but there was no sort applied!"
      );
    }
  };
  ...
  const setCurrentSortColumn = (columnIndex: number) => {
    ...
    if (tableColumns() === undefined) {
      console.error(
        "[-] Tried to call setCurrentSortColumn but tableColumns() is undefined! - ",
        tableColumns()
      );
      return;
    }
    ...
    if (columnIndex > tableColumns()!.length) {
      console.error(
        "[-] Tried to set a sort column, for a column that doesnt exist, columnIndex was > tableColumns().length"
      );
      return;
    }
        ...
        case StringSort.Alphabetical: {
          ...
          tableCopy.sort((a, b) => {
            if (a.item.hasOwnProperty(key) && b.item.hasOwnProperty(key)) {
              if (
                typeof a.item[key] === "string" &&
                typeof b.item[key] === "string"
              ) {
                return (a.item[key] as string).localeCompare(
                  b.item[key] as string
                );
              } else {
                return 0;
              }
          ...
          console.log(
            "[+] This is the copy of the new data to sort.",
            tableCopy
          );
  ...
  createEffect(() => {
    if (currentSort() !== previousSort()) {
      console.log(
        "[+] Effect: Sort Has Changed, Setting Previous Sort, Setting New Current Sort"
      );
  ...
  const PopoverContent: Component<PopoverContentProps> = (
                  ...
                  <tr>
                    ...
                    <td>
                      <div class={"cell-inner"}>
                        <PopoverContent
                          item={residentItem}
                          cellContent={residentItem.name}
                        />
                      </div>
                    </td>
                    <td>
                      <div class={"cell-inner"}>{residentItem.doc}</div>
                    </td>
                    <td>
                      <div class={"cell-inner"}>{residentItem.room}</div>
                    </td>
                    <td>
                      <div
                        class={`cell-inner ${
                          residentItem.current_location === residentItem.unit
                            ? "bg-green-400"
                            : "bg-red-400 text-white"
                        }`}
                      >
                        <GetLocationById id={residentItem.current_location} />
                      </div>
                    </td>
                    {props.actions ? (
                      <td>
              ...
              {(item) => {
                ...
                return (
                  <tr>
                    ...
                    <td>
                      <div class={"cell-inner"}>{locationItem.id}</div>
                    </td>
                    <td>
                      <div class={"cell-inner"}>{locationItem.name}</div>
                    </td>
                    <td>
                      <div class={"cell-inner"}>
                        <For each={props.actions}>
                    ...
                    <tr class={"text-white"}>
                      ...
                      <td class={"bg-red-400"}>
                        <div class={"cell-inner"}>
                          <PopoverContent
                            item={timestampItem}
                            cellContent={timestampItem.name}
                          />
                        </div>
                      </td>
                      <td class={"bg-red-400"}>
                        <div class={"cell-inner"}>{timestampItem.doc}</div>
                      </td>
                      <td class={"bg-red-400"}>
                        <div class={"cell-inner"}>{timestampItem.room}</div>
                      </td>
                      <td class={"bg-red-400"}>
                        <div class={"cell-inner text-sm"}>
                          {timestampItem.timestampLeft}
                        </div>
                      </td>
                      <td class={"bg-red-400"}>
                        <div class={"cell-inner"}>
                          {timestampItem.destinationLabel}
                        </div>
                      </td>
                    </tr>
                  );
              ...
              {(item) => {
                ...
                return (
                  <tr>
                    ...
                    <td>
                      <div class={"cell-inner"}>{timestampItem.rfid}</div>
                    </td>
                    <td>
                      <div class={"cell-inner"}>
                        {timestampItem.destinationId}
                      </div>
                    </td>
                    <td>
                      <div class={"cell-inner"}>{timestampItem.timestamp}</div>
                    </td>
                    <td>
                      <div class={"cell-inner"}>
                        <For each={props.actions}>

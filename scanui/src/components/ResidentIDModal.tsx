...
export const ResidentIDModal: Component<ResidentIDModalProps> = (
  props: ResidentIDModalProps,
) => {
  const [newResidentHousingUnit, setNewResidentHousingUnit] = createSignal<
    number | string | 1 | 6 | 7 | 14 | 17
  >(1);
  const [newResidentRoom, setNewResidentRoom] = createSignal<number | string>(
    0,
  );
  const [newResidentBunk, setNewResidentBunk] = createSignal<"T" | "B" | null>(
    null,
  );
  const [newResidentFirstName, setNewResidentFirstName] =
    createSignal<string>("");
  const [newResidentLastName, setNewResidentLastName] =
    createSignal<string>("");
  const [newResidentDOC, setNewResidentDOC] = createSignal<string>("");
  const [newResidentPod, setNewResidentPod] = createSignal<string>("");
  const [newResidentLevel, setNewResidentLevel] = createSignal<number>(4);

  const [newResidentValidationErrors, setNewResidentValidationErrors] =
    createSignal<ResidentIDModalValidationErrors>({
      firstNameError: null,
      lastNameError: null,
      docError: null,
  ...
  const handleChangeLevel = (e: any) => {
    setNewResidentLevel(e.currentTarget.value);
  };
  ...
  const validateNewResident = () => {
    ...
    if (
      housingUnits.includes(parseInt(newResidentHousingUnit() as string)) ===
      false
    ) {
      errors = {
        ...errors,
        housingUnitError:
          "Housing Unit is invalid! " + newResidentHousingUnit().toString(),
      };
    }
    if (newResidentLevel() === null || newResidentLevel() < 0) {
      errors = {
        ...errors,
  ...
  const housingUnitLetter = (
    housingUnit: number,
  ): string | "A" | "B" | "C" | "D" | "E" | undefined => {
  ...
  const housingRoom = (room: number) => {
    ...
    let housingRoom =
      housingUnitLetter(newResidentHousingUnit() as number) + roomString;
  ...
  const createNewResident = () => {
    ...
    if (
      newResidentValidationErrors().bunkError !== undefined ||
      ...
      newResidentValidationErrors().levelError !== undefined
    ) {
    ...
    Object.values(newResident).forEach((value, index) => {
      if (value === null || value === undefined || value === "") {
        alert(
          "Whoops!, the resident payload had an empty or invalid value: " +
            value +
            " index:" +
            index,
        );
    ...
    return (
      <Show when={props.open}>
        <Portal mount={document.body}>
          <div
            class={
              "fixed top-0 left-0 right-0 bottom-0 bg-black opacity-70 z-40"
            }
          />
          <div
            class={
            ...
            <section class="flex flex-col gap-5 w-full h-fit bg-slate-50 px-10 py-3">
                ...
                <div class={"flex w-full"}>
                  <div class="flex flex-col gap-2 w-full">
                    ...
                    <input
                      ...
                      type={"text"}
                      id={"newResidentFirstName"}
                      name={"newResidentFirstName"}
                      placeholder="First Name"
                    />
                    {newResidentValidationErrors().firstNameError !== null ? (
                      <span class={"text-red-500"}>
                        {newResidentValidationErrors().firstNameError}
                      </span>
                    ) : (
                      false
                    )}
                ...
                <div class={"flex w-full"}>
                  <div class="flex flex-col gap-2 w-full">
                    ...
                    <input
                      ...
                      class={"bg-white border-[1px] py-3 px-3"}
                      type={"text"}
                      id={"newResidentLastName"}
                      name={"newResidentLastName"}
                      placeholder="Last Name"
                    />
                    {newResidentValidationErrors().lastNameError !== null ? (
                      <span class={"text-red-500"}>
                        {newResidentValidationErrors().lastNameError}
                      </span>
                    ) : (
                      false
                    )}
              ...
              <section class={"flex w-full h-fit"}>
                <div class={"flex flex-col gap-2 w-full"}>
                  ...
                  <input
                    ...
                    name={"newDOC"}
                  />
                  {newResidentValidationErrors().docError !== null ? (
                    <span class={"text-red-500"}>
                      {newResidentValidationErrors().docError}
                    </span>
                  ) : (
                    false
                  )}
              ...
              <section class={"flex w-full h-fit gap-4 columns-3"}>
                <div class={"flex gap-2 w-full"}>
                  ...
                  {newResidentValidationErrors().housingUnitError !== null ? (
                    <span class={"text-red-500"}>
                      {newResidentValidationErrors().housingUnitError}
                    </span>
                  ) : (
                    false
                  )}
                ...
                <div class={"flex gap-2 w-full"}>
                  ...
                  {newResidentValidationErrors().roomError !== null ? (
                    <span class={"text-red-500"}>
                      {newResidentValidationErrors().roomError}
                    </span>
                  ) : (
                    false
                  )}
                ...
                <div class={"flex gap-2 w-full"}>
                  <label class={"flex flex-col w-full"}>
                    ...
                    <select
                      value={newResidentBunk()?.toUpperCase() as string}
                  ...
                  {newResidentValidationErrors().bunkError !== null ? (
                    <span class={"text-red-500"}>
                      {newResidentValidationErrors().bunkError}
                    </span>
                  ) : (
                    false
                  )}

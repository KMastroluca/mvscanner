...
interface ResidentIDModalState {
   open: Accessor<boolean>;
   newResidentRoomCharacter:Accessor<string|null>;
   newResidentHousingUnit:Accessor<ResidentIDModalHousingUnit>;
}
...
export const ResidentIDModal:Component<ResidentIDModalProps> = (props:ResidentIDModalProps) => {
   ...
   const Modal:Component<ResidentIDModalState> = (state:ResidentIDModalState) => {

      return (
      <Show when={state.open()}>
         <Portal mount={document.body}>
            <div class={"fixed top-0 left-0 right-0 bottom-0 bg-black opacity-70 z-40"} />
            <div class={"flex flex-col w-3/5 h-3/4 fixed top-[50%] left-[50%] bg-white translate-x-[-50%] translate-y-[-50%] z-50"}>
               ...
               <section class="flex flex-col gap-5 w-full h-fit bg-slate-50 px-10 py-3">
                  <section class={"flex w-full h-fit"}>
                     <div class={"flex flex-col gap-2 w-full"}>
                        ...
                        <input type={"text"} disabled class={"bg-neutral-200 border-[1px] py-3 px-3"} id={"newRFID"} name={"newRFID"} />
                     </div>
                  </section>
                  <section class={"flex w-full h-fit columns-2 gap-4"}>
                     <div class={"flex w-full"}>
                        <div class="flex flex-col gap-2 w-full">
                           ...
                           <input class={"bg-white border-[1px] py-3 px-3"} type={"text"} id={"newResidentFirstName"} name={"newResidentFirstName"} placeholder="First Name" />
                        </div>
                     </div>
                     <div class={"flex w-full"}>
                        <div class="flex flex-col gap-2 w-full">
                           <label for={"newResidentLastName"} class={"text-lg font-bold"}>Resident Last Name:</label>
                           <input class={"bg-white border-[1px] py-3 px-3"} type={"text"} id={"newResidentLastName"} name={"newResidentLastName"} placeholder="Last Name" />
                        </div>
                     </div>      
                  </section>
                  <section class={"flex w-full h-fit gap-4 columns-3"}>
                        ...
                        <div class={"flex gap-2 w-full"}>
                           <label class={"flex flex-col w-full"}>
                              ...
                              <select name={"newResidentRoom"} class={"px-3 py-3 border-[1px]"} onChange={(e) => handleChangeRoom(e)}>
   ...
   return (
         <Modal open={showModal} newResidentHousingUnit={newResidentHousingUnit} />
   );
